...
int xr_mysql::mysql_if_t::connect_server()
{
	...
	{
		ALERT_LOG("db connect is err code:%d, %s", this->get_errno(), this->get_error());
		return xr::ERROR::DB_SYS;
	}
	INFOR_LOG("db connect is ok code:%d", this->get_errno());
...
int xr_mysql::mysql_if_t::execsql(const char *sql, int len)
{
	...
	if (SUCC != ret)
	{
		int e = this->get_errno();
		if (CR_SERVER_GONE_ERROR == e)
...
int xr_mysql::mysql_if_t::close_db_autocommit()
{
	if (SUCC != mysql_autocommit(&this->handle, 0))
	{
		ALERT_LOG("db mysql_autocommit close is err code:%d, %s", this->get_errno(), this->get_error());
		return xr::ERROR::DB_SYS;
	}
	return SUCC;
}
...
int xr_mysql::mysql_if_t::commit()
{
	...
	{
		ALERT_LOG("db mysql_commit is err code:%d, %s", this->get_errno(), this->get_error());
		return xr::ERROR::DB_SYS;
	}
	return SUCC;
}
